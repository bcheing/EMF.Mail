...
namespace EMF.Mail.Services
{
    ...
    public class MessageProcessor(DataService mailDataSvc, FilerDataService filerDataSvc, CommandService cmdSvc, ClaudeClassifier classifier, Filer filer, ConversationService conv)
    {
        ...
        // actually written.
        private async Task FinalizeAsync(MessageFinalize finalize)
        {
            var result = await mailDataSvc.FinalizeMessageAsync(finalize);
            if (result.IsFailure)
        ...
        private async Task ProcessSelfSentMessageAsync(MailAccount account, Message message)
        {
            ...
            await FinalizeAsync(new MessageFinalize { MsgNo = msgResult.Value.MsgNo, ResTpCode = "NOOP", MsgResult = "Message from the account's own mailbox address, not a real submission." });
        ...
        private async Task ProcessClassifiedMessageAsync(MailAccount account, IMailService mail, Message message, int msgNo, int senderId, List<MsgType> msgTypes, Dictionary<int, List<ClaudeFieldSpec>> fieldsByMsgType, ClassifyResult result, bool saveContext)
        {
            ...
            if (result.MsgTpCode is null)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, ResTpCode = "REVIEW", MsgResult = "Could not determine message type." });
            ...
            if (msgType is null)
            {
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "FAILED", MsgResult = $"No message type configured for {result.MsgTpCode}." });
            ...
            if (result.MsgTpCode == "INQ")
            {
                ...
                {
                    await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "FAILED", MsgResult = $"No GetHndName configured for message type {result.MsgTpCode}." });
                ...
                if (reply.IsFailure)
                {
                    await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "FAILED", MsgResult = $"Failed to compose inquiry reply: {reply.Message}" });
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "OK", MsgResult = $"Inquiry reply sent ({matches.Count} match(es) found)." });
            ...
            {
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "FAILED", MsgResult = $"No PutHndName configured for message type {result.MsgTpCode}." });
            ...
            foreach (var pkg in result.Packages)
            {
                ...
                {
                    var putResult = await mailDataSvc.PutRecordAsync(msgType.PutHndName, pkg.Fields);
            ...
            {
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = context, MsgTpCode = result.MsgTpCode, ResTpCode = "NOOP", MsgResult = "No packages created from this message." });
        ...
        private async Task ProcessInboundMessageAsync(MailAccount account, IMailService mail, List<DocType> docTypes, List<MsgType> msgTypes, Dictionary<int, List<ClaudeFieldSpec>> fieldsByMsgType, string processDesc, Message message)
        {
            ...
            if (classifyResult.IsFailure)
            {
                await mail.MarkNeedsReviewAsync(message.ProvMsgId);
                await mail.ForwardAsync(message.ProvMsgId, account.AdmAcctEMail, "This message could not be classified and needs manual review.");
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, ResTpCode = "REVIEW", MsgResult = $"Claude classification failed: {classifyResult.Message}" });
            ...
            if (!isLinked)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgContext = result, MsgTpCode = result.MsgTpCode, IsHeld = true, FwdMsgId = fwdMsgId, ResTpCode = resTpCode, MsgResult = msgResultText });
        ...
        private async Task ProcessInfoRequestReplyAsync(MailAccount account, IMailService mail, Message message, RfiBridgeResult bridge, List<DocType> docTypes, string processDesc)
        {
            ...
            if (remaining.Count == 0)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgTpCode = "SUB", IReqNo = bridge.IReqNo, ResTpCode = "OK", MsgResult = "RFI resolved, all items received." });
            ...
            else
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = msgNo, MsgTpCode = "SUB", IReqNo = bridge.IReqNo, ResTpCode = "REVIEW", MsgResult = $"{remaining.Count} item(s) still outstanding after RFI reply; closed without re-asking, admin notified." });
        ...
        private async Task ReprocessHeldMessageAsync(MailAccount account, IMailService mail, HeldMessage heldMsg, int senderId, List<MsgType> msgTypes, Dictionary<int, List<ClaudeFieldSpec>> fieldsByMsgType)
        {
            ...
            if (orig is null)
            {
                await FinalizeAsync(new MessageFinalize { MsgNo = heldMsg.MsgNo, ResTpCode = "FAILED", MsgResult = "Approved but original message could not be refetched from Inbox." });
            ...
            if (result is null)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = heldMsg.MsgNo, ResTpCode = "REVIEW", MsgResult = "Saved MsgContext missing or unreadable on reprocess." });
        ...
        private async Task ProcessAdminReplyAsync(MailAccount account, IMailService mail, Message message, List<MsgType> msgTypes, Dictionary<int, List<ClaudeFieldSpec>> fieldsByMsgType)
        {
            ...
            if (held is null)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "REVIEW", MsgResult = "No matching hold found." });
            ...
            if (cmdResult.IsFailure)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "FAILED", MsgResult = $"Claude command interpretation failed: {cmdResult.Message}" });
            ...
            if (cmdResult.Value.CmdCode != "APPROVE" && cmdResult.Value.CmdCode != "REJECT")
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "REVIEW", MsgResult = "Admin reply did not resolve to APPROVE/REJECT." });
            ...
            if (cmdResult.Value.VendorName is not null && !string.Equals(cmdResult.Value.VendorName, held.CandVendName, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (matches.Count != 1)
                {
                    ...
                    await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "REVIEW", MsgResult = $"Vendor \"{cmdResult.Value.VendorName}\" not found (or ambiguous)." });
            ...
            if (vendId is null)
            {
                ...
                await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "REVIEW", MsgResult = "No vendor could be resolved." });
            ...
            if (isApproved)
            {
                ...
                if (linkResult.IsFailure)
                {
                    ...
                    await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "FAILED", MsgResult = $"Failed to link vendor: {linkResult.Message}" });
            ...
            await FinalizeAsync(new MessageFinalize { MsgNo = adminMsgNo, MsgTpCode = "CMD", ResTpCode = "OK" });
        }
        ...
        public async Task ProcessAccountAsync(MailAccount account, IMailService mail)
        {
            ...
            if (messages.Count > 0)
            {
                ...
                var msgTypesTask = mailDataSvc.GetMsgTypesAsync(account.AppId);
                var msgTypeFieldsTask = mailDataSvc.GetMsgTypeFieldsAsync(account.AppId);
